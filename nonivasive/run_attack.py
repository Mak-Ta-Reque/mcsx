...
from nn.utils import get_expl, plot_overview, clamp, load_image, make_dir, get_robust_expl
...
def main():
    ...
    argparser.add_argument('--img', type=str, default='../imgs/horse.jpg', help='image net file to run attack on')
    argparser.add_argument('--target_img', type=str, default='../imgs/horse.jpg',
    ...
    argparser.add_argument('--method', help='algorithm for expls',
                           ...
                           default='gradient')
    argparser.add_argument('--robust', action='store_true', help='''
        If set, the program will use a robust algorithm.
        ''')
    argparser.add_argument('--noisy', action='store_true', help='''
        If set, the program will add gausin noise in the input.
        ''')
    ...
    target_expl, _, trg_inx = get_expl(model, x_target, method)
    ...
    adv_expl, adv_acc, class_idx = explainer(model, x_adv, method)
    xl, adv_acc, class_idx = explainer(model, x, method)
    #print("Org: ",  org_idx.item())
    #print("Trg: ",  org_idx.item())
    #print("robust: ",  class_idx.item())
    # save results
    output_dir = make_dir(args.output_dir)
    
    plot_overview([x_target, x, x_adv], [target_expl, xl, adv_expl], data_mean, data_std, filename=f"{output_dir}/overview_{args.method}.pdf")
