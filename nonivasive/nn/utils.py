...
def get_expl(model, x, method, desired_index=None):
    ...
    if method == ExplainingMethod.integrated_grad:
        ...
        guided_gc = LayerGradCam(model , model.features[30])
        heatmap = guided_gc.attribute(x, class_idx.item())
        heatmap = LayerAttribution.interpolate(heatmap, (224, 224), interpolate_mode='bicubic')
...
def get_robust_expl(model, x_adv, method, desired_index=None, num_samples=6):
    """
    Helper method to get the heatmap
    """
   
    acc, class_idx = model.classify(x_adv)
    model.train()  # set model to training mode to enable dropout
    x = copy.deepcopy(x_adv)
    x.requires_grad = True
    #acc, class_idx = model.classify(x)
    if desired_index is None:
        desired_index = class_idx
    ...
    for _ in range(num_samples):
        ...
        if method == ExplainingMethod.integrated_grad:
            ...
            # we use DataParallel mode of model to fit the batch in memory of (multiple) gpu(s)
            num_summands = 30
            prefactors = noisy_x.new_tensor([k / num_summands for k in range(1, num_summands + 1)])
            parallel_model = torch.nn.DataParallel(model)
            parallel_model.train()
            y = parallel_model(prefactors.view(num_summands, 1, 1, 1) * noisy_x)

            # we sum the result and then take the derivative (instead of summing derivatives as in most implementations),
            # (d/dx) (n*y_1(1/n*x) + n/2*y_1(2/n*x) .... + y_n(x) ) = y_1'+....y'_n
            y = torch.nn.functional.softmax(y, 1)[:, int(desired_index)]
            y = (1 / num_summands) * torch.sum(y / prefactors, dim=0)
            heatmap = torch.autograd.grad(y, noisy_x, create_graph=True)[0]
        else:
            heatmap = model.analyze(method=method, R=None, index=desired_index)

        if method == ExplainingMethod.grad_times_input or method == ExplainingMethod.integrated_grad:
            heatmap = heatmap * noisy_x

        heatmap = torch.sum(torch.abs(heatmap), dim=1)

        normalized_heatmap = heatmap / torch.sum(heatmap)
        normalized_heatmap = normalized_heatmap.squeeze()
        #print(normalized_heatmap.size())
        
        heatmaps.append(normalized_heatmap)
    #print(len(heatmaps))
    heatmaps = torch.stack(heatmaps, dim=0)
    #print(heatmaps.size())
    #heatmap = torch.abs(torch.mean(heatmap, dim=1))
      

    # Monte Carlo Dropout

    heatmap = torch.mean(heatmaps, dim=0)
    #normalized_heatmap = max_hist(heatmaps)
