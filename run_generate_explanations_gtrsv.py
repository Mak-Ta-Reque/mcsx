...
import utils
from models import load_model, load_resnet20_model_normal,  load_gtsrb_model_normal
...
from plot import generate_explanation, generate_gtrsb
...
def testable_evaluate_models(attackid:int,resultdir,metric,batchsize,nsim, robust=False, hist=False):
    ...
    original_model = load_model("resnet20_gtsrb",0)
    manipulated_model = load_gtsrb_model_normal(attack_folder / "model.pth", os.environ["CUDADEVICE"], state_dict=False,keynameoffset=7,num_classes=43)
    ...
    x_test, label_test, *_ = load_data(utils.DatasetEnum.GTSRB, test_only=True, shuffle_test=False)
    ...
    generate_gtrsb.generate_explanation_and_metrics(resultdir, metric,run.get_epochs(), original_model, manipulated_model, x_test, label_test, run, batchsize, nsim = nsim, hist=hist, save=False, show=False, robust=robust)
...
def main():
    ...
    os.environ['MODELTYPE'] = "resnet20_gtsrb_normal"
...
#example run cmd is python run_generate_explanations.py 54 --resultdir /mnt/sdb/abka03-data/gtsrv/nondropout --metric mse --batchsize 500 --device cuda:0 --nsim 2 --robust
