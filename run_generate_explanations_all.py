...
import utils
from models import load_model, load_manipulated_model, resnet_freeze_bn
...
from plot import  generate_gtrsb as generate
...
def testable_evaluate_models(attackid:int,resultdir,metric, datasize, batchsize,nsim, robust=False, hist=False):
    """
    ...
    x_test, label_test = x_test[0:datasize], label_test[0:datasize]
    
    print(f"Loaded")
    
    # Load the manipulated model
    print(f"Loading models...")
    original_model = load_model(params["modeltype"],7)
    manipulated_model = load_manipulated_model(attack_folder, which=params["modeltype"])
    manipulated_model.set_softplus(beta=1)
    # specificaly for batch freezed model 
    #manipulated_model.apply(resnet_freeze_bn.set_bn_params)
    print(f"Loaded")
    #print( manipulated_model)
...
def main():
    ...
    parser.add_argument('--batchsize',type = int, help='''
        ...
        ''')
    parser.add_argument('--datasize', type = int, default= 10000, help='''
        batch size to run the algorithm
        ''')
    ...
    testable_evaluate_models(attackid,resultdir,metric, datasize, batchsize, nsim,robust=robust, hist = args.hist)
...
# python run_generate_explanations_all.py 198 --resultdir /mnt/sdc/abka03-data/cifar10/batch_test_bn/bs36 --metric mse --datasize 1000 --batchsize 36 --device cuda:0 --nsim 2 --robust
